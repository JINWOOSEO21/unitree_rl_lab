...
### 취소: "종료 hang" 은 브리지 결함이 아니었다

앞서 이 문서와 커밋에 "루프 종료 후 약 55 초 매달린다" 고 적었는데 **틀렸다**. 계측 없이
순수 브리지만 재면 `--duration 10` 이 `wall 15.7 s, exit 0` 으로 두 번 연속 깨끗하게 끝난다
(나머지 5.7 초는 import + CUDA 초기화 + warmup).

원인은 `tools/go2_bridge_stage_probe.py` 였다. `time.sleep` 을 **락을 잡는** 파이썬 함수로
전역 패치했는데, CycloneDDS 종료 스레드도 그 래퍼를 부르고 이 도구의 데몬 리포터가 같은
락을 쥐고 있어 교착했다. `--stacks` 로 본 segfault 도 같은 계통이다 — faulthandler 가 C
확장 안에서 실행 중인 스레드 스택을 덤프하다 죽는다. 도구를 락 없는 카운터로 고쳤고
docstring 에 두 함정을 적어 두었다.

**측정 도구가 만든 증상을 대상의 결함으로 보고하지 않도록, 이상을 발견하면 계측을 빼고
한 번 더 재는 것을 절차로 둔다.**

### 남은 것

- 기립 `foot_force` 가 27~30 (원시 정수) 으로 임계 20 에 가깝다. `LegOdomCfg` 주석의
  "실기 foot_force 는 원시 정수라 재보정 필요" 가 아직 미결이다. 엎드림 기준선은
  `[7,5,5,6]` 인데 다른 전원 주기에서는 `[4,2,4,4]` 로 달랐다 — 절대 임계 하나로 네 발을
  판정하는 현재 구조의 여유가 작다.
- 단계 C-4: bridge 재시작 시 session 교체와 controller 수신부 복구, Policy 중 bias
  snapshot 고정 / 비Policy 중 candidate 갱신 검증. `go2_gyro_bias_dds_probe`(loopback
  domain 181, `lo`) 가 이미 그 시나리오를 판정하지만 **짝이 되는 파이썬 발행 드라이버가
  없다** — C-4 에서 만들어야 한다.
- 지금까지 모든 측정이 **기립 정지** 상태다. 주행 중 leg odometry 정확도는 미검증이며
  단계 E 의 짧은 직진에서야 나온다.

## 단계 D 1차 시도: AP 로는 현재 구성에서 안 된다 (2026-09-17)

노트북을 Go2 AP(`ESSID "blue"`, 5.18 GHz)에 붙이면 **192.168.12.0/24** 를 받는다.
로봇 내부망은 **192.168.123.0/24** 라 서브넷이 갈린다. 유선은 인터넷으로 돌렸다.

### 1. 무선 링크 자체는 좋다 — 단 절전을 꺼야 한다

| | 기본값 | `iw dev wlo1 set power_save off` 후 |
|---|---|---|
| RTT min/avg/max | 3.6 / **29.9** / **117.5** ms | 2.0 / **3.2** / **15.2** ms |
| 편차 | 36.9 ms | 2.8 ms |

신호 −37 dBm, rx 200 / tx 108 Mbit/s 로 품질은 훌륭하다. **절전이 켜져 있으면 지연이
3.6→117 ms 로 요동쳐 유선의 scandots age 85 ms 기준조차 못 지킨다.** 대역폭 문제가 아니다
(scandots 는 132 float + 헤더 × 10 Hz ≈ 6 KB/s). 무선을 쓴다면 이 설정이 전제다.

### 2. 라우팅은 절반만 뚫린다

AP 서브넷에는 게이트웨이(192.168.12.1)와 노트북밖에 없다. Jetson 은 192.168.12.x 에
**없다**(`192.168.12.18` ARP 무응답). `ip route add 192.168.123.0/24 via 192.168.12.1` 을
넣으면:

| 대상 | 결과 |
|---|---|
| 192.168.123.161 (제어보드) | ping 3/3, avg 2.97 ms / TCP 22 는 `connection refused` = **도달** |
| 192.168.123.18 (Jetson) | ICMP 100 % loss, TCP 22 무응답 = **도달 불가** |

제어보드는 게이트웨이 자신일 가능성이 높아 전달 없이 응답한다. Jetson 까지 가려면
게이트웨이가 내부망으로 **전달**하고 Jetson 에 192.168.12.0/24 **반환 경로**가 있어야
하는데 둘 중 하나가 없다. 어느 쪽인지는 Jetson 에 들어가야 갈린다 —
**그런데 AP 만으로는 Jetson 에 ssh 조차 안 된다.**

### 3. DDS 는 아예 오지 않는다

```
wlo1 에서 239.255.0.1:7400 10 초 청취  ->  {}            (discovery 패킷 0)
go2_state_probe wlo1                   ->  NO DATA       (LowState 0)
+ AllowMulticast false, Peers 192.168.12.1 / 192.168.123.161  ->  NO DATA
```

멀티캐스트는 라우터를 넘지 않고(계획 D-3 의 경고 그대로), unicast peer 를 명시해도 제어보드
DDS 가 응답하지 않는다. 제어보드는 DDS 를 192.168.12.0/24 에 내보내지 않는 것으로 보인다.

### 결론

계획 D-7 의 조건("AP 로 경로가 확보되지 않으면 유선 유지")에 해당한다. **유선 구성을
유지한다.**

조사 결과 이것은 설정 문제가 아니라 **구조 문제**였다. 내장 AP 의 용도는 앱을 통한
WebRTC 제어이고, 그 경로는 `rt/lowcmd` 를 설계상 나르지 않는다 — 네트워크를 아무리
뚫어도 저수준 제어는 그 길로 못 간다. 무선으로 저수준 제어를 하는 사람들은 내장 AP 를
쓰지 않고 **192.168.123.0/24 에 AP 를 붙여** 노트북을 로봇과 같은 L2 에 넣는다.
그 구성이면 우리 코드는 한 줄도 바뀌지 않는다.

→ **조사 전문·대안 비교·다음 검증 절차: [`go2_wireless_options.md`](go2_wireless_options.md)**
